...
这个格式是内核契约：
- `market_ohlcv` 返回的永远是这个格式（不管底层适配器是什么）
...
```
market_ohlcv(symbol, interval, mode, start, end)
       │
       ▼
  ┌─ MarketAdapter (内核接口) ─────────────┐
  │  fetch(query)                           │
  │  → canonical OHLCV + meta              │  ← 内核保证归一化
  └─────────────────────────────────────────┘
       │ (具体实现可替换)
       ├─ TushareAdapter (默认)
       ├─ YFinanceAdapter（默认美股 / 可选 A 股）
       └─ CSVAdapter (可选，本地文件)
```
...
`market_ohlcv` 不能用 read/write 替代——它需要 MarketAdapter 归一化 + canonical 格式契约 + OHLCV→DataStore→compute 预连通管道。
...
| OHLCV → compute | 眼睛看到 → 大脑可用 | compute 调用时 | 自动将最近一次或指定 selector 的 market_ohlcv 结果注入执行环境 |
...
market_ohlcv(
    symbol: str,
    interval: Literal["1d", "1m", "5m", "15m", "30m", "60m"] = "1d",
...
```python
# 数据（由内核从最近一次或指定 selector 的 market_ohlcv 结果注入）
...
1. 内核内置 — 需要内核级集成（安全/状态/预连通），不可外部提供
   market_ohlcv    适配器归一化 + canonical format + selector-aware DataStore
...
3. Skill 编排 — 工作流，不提供新能力，只组织已有工具
   /skill:research     调 market_ohlcv + compute + write + recall
...
| 技能 | 命令 | 工作流 |
|------|------|--------|
| 研究 | `/skill:research {topic}` | market_ohlcv → compute 指标 → 结合 memory → notebook 写分析笔记 |
...
Agent 自动编排：
  1. market_ohlcv(symbol="300750.SZ", interval="1d", mode="history") → 获取近期行情
  2. compute(code="ta.rsi(close, 14)", symbol="300750.SZ", interval="1d", mode="history") → 计算 RSI（OHLCV 自动注入）
