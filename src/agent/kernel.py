...
from agent.messages import ContextRef, TurnInput, build_user_message, ensure_turn_input, normalize_history, render_turn_input
...
memory.md — 你关于用户和世界的长期记忆。
  ✅ 写入：用户称呼、投资偏好、风险偏好、关注市场、关注方向、持仓快照、长期计划、研究结论、教练式反馈事件
  ✅ 投资场景重点关注：当用户说出自己的持仓、成本、仓位、关注方向、风险边界或长期目标时，默认主动记入 memory.md
  ❌ 不写：你自己的信念或方法论（那是 soul 的内容）
  组织：单文件，newest-first 倒排。持仓信息按新快照追加，保留历史，不覆盖旧记录。
  读取：按需主动读取。当任务涉及个性化建议、历史延续、持仓分析、复盘、风格匹配、矛盾纠偏时，先读最近记忆。
...
- 投资相关高价值信息默认主动记入 memory.md，不必等用户再次提醒。
