...
`models.py`: 所有 dataclass 数据结构，无业务逻辑，全模块共享基础层；SlippageConfig 支持 fixed/pct 模式 + 成交量约束
`engine.py`: 确定性市场模拟，多资产 dict 数据/limit+stop 订单/bracket OCO/做空/风控4检查/百分比滑点/部分成交
`indicators.py`: IndicatorEngine，pandas-ta 防前瞻包装，calc/list_indicators
`memory.py`: 文件式记忆系统，Workspace 隔离 + Memory(log/note/recall)
`tools.py`: ToolKit，OpenAI function calling schema + 工具分发 + 调用追踪；market_observe 支持 symbol 参数
