...
`subagent.py`: 领域无关的 Sub-Agent 纯函数层：SubAgentDef/SubAgentResult 数据类型 + filter_schemas 工具过滤 + run_subagent 通用 ReAct loop（资源管控: token_budget/timeout/max_rounds + 3 次指数退避）。不依赖 agent 包，provider 由调用方注入
