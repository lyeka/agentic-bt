...
memory.md — 你关于用户和世界的长期记忆。
  ...
  ✅ 投资场景重点关注：当用户说出自己的持仓、成本、仓位、当前自选、关注方向、风险边界或长期目标时，默认先判断是否该写入 portfolio.json 或 watchlist.json；memory.md 只保留高层背景和长期偏好
...
watchlist.json — 用户的结构化自选列表快照。
  ✅ 写入：当前关注标的、观察理由、加入自选时间
  ✅ 触发：用户说加入自选、移出自选、给出完整自选列表，或要求围绕某个理由持续跟踪标的
  ❌ 不写：当前持仓、长篇研究日志、价格提醒规则、无法可靠识别 symbol 的内容
  组织：单文件 JSON，按 list_id 维护当前有效观察对象，不记录历史日志。
  读取：当任务涉及自选复盘、加入后走势、开盘前巡检或围绕观察理由跟踪标的时，优先调用 watchlist 工具读取。
...
- 用户透露持仓、成本、仓位、当前自选、关注方向、风险偏好、投资目标
...
- 投资相关高价值信息默认主动落到正确的地方：详细当前持仓进 portfolio.json，具体自选列表进 watchlist.json，高层偏好和长期背景进 memory.md。
...
- 把具体自选 symbol 清单继续塞进 memory.md，而不是用 watchlist 工具
- 把未核实的数据写入长期记忆
</workspace>"""
...
TRADE_GUIDE = """\
<trade_tools>
当你要读取或执行远端 broker 交易动作时，严格遵循以下规则：
...
class Kernel:
    ...
    def request_confirm(self, message: str) -> bool:
        """请求用户确认。无 handler 时默认放行（yolo）"""
        if self._confirm_handler is None:
            return True
        return self._confirm_handler(message)
