...
[POS]: AthenaClaw compute 工具的共享计算沙箱；主线程 signal 超时 / 非主线程 ThreadPoolExecutor 降级
...
except ModuleNotFoundError:  # pragma: no cover - fallback is exercised in test env
    class _FallbackTA:
        """Small compatibility layer for environments without pandas_ta."""
        ...
        @staticmethod
        def sma(close: pd.Series, length: int = 10) -> pd.Series:
        ...
        @classmethod
        def bbands(cls, close: pd.Series, length: int = 20, std: float = 2.0) -> pd.DataFrame:
            middle = cls.sma(close, length=length)
        ...
        def macd(
            ...
            macd_line = cls.ema(close, length=fast) - cls.ema(close, length=slow)
        ...
        def __getattr__(self, name: str) -> Any:
            raise ImportError(
                "pandas_ta is not installed; fallback only provides "
                "ema/sma/rsi/bbands/macd."
            )
