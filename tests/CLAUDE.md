...
`features/kernel_tools.feature`: Kernel 工具与工作区行为规格（14 scenarios：market_ohlcv interval/mode/start/end 协议 + DataStore selector + compute 选帧/跨 symbol 防误取 + read/write/edit/权限/Session 持久化/自举）
`test_kernel_tools.py`: kernel_tools.feature step definitions（fixture: ktctx，直接调用 handler，CsvAdapter 多周期桩 + SpyAdapter 记录 query selector）
...
`features/tushare_adapter.feature`: TushareAdapter 行为规格（6 scenarios：1d/history/分钟 history/latest/排序/权限错误/`.SS`→`.SH` 归一化）
`test_tushare_adapter.py`: tushare_adapter.feature step definitions（fixture: tsctx，mock tushare API，覆盖 `stk_mins`/`rt_min_daily`）
`features/yfinance_adapter.feature`: YFinanceAdapter 行为规格（5 scenarios：1d/history/分钟 history/latest/`.SH`→`.SS` 归一化/分钟范围透传）
`test_yfinance_adapter.py`: yfinance_adapter.feature step definitions（fixture: yfctx，mock yfinance Ticker.history）
`features/finnhub_adapter.feature`: FinnhubAdapter 行为规格（6 scenarios：1d/history 标准化/UNIX时间戳透传/默认范围/分钟与 latest 显式拒绝）
`test_finnhub_adapter.py`: finnhub_adapter.feature step definitions（fixture: fhctx，mock finnhub client）
`features/market_routing.feature`: CompositeMarketAdapter 路由行为规格（5 scenarios：基于 normalized symbol 的匹配路由/fallback/first-match-wins/无fallback异常）
`test_market_routing.py`: market_routing.feature step definitions（fixture: mrctx，FakeAdapter 返回 MarketFetchResult 纯路由逻辑）
...
`test_discord_adapter.py`: Discord 适配器 helper 单测（allowlist/bool 解析 + 附件采集 + backend 基础能力）
