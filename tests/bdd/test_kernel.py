...
def test_workspace_guide_emphasizes_growth_and_investment_memory():
    ...
    assert "持仓、成本、仓位、当前自选、关注方向、风险偏好、投资目标" in WORKSPACE_GUIDE
    ...
    assert "watchlist.json" in WORKSPACE_GUIDE
    assert "具体自选列表进 watchlist.json" in WORKSPACE_GUIDE
...
def test_seed_prompt_bootstraps_on_first_durable_signal():
    ...
    assert "长期偏好、持仓、当前自选、关注方向、风险边界、投资目标" in SEED_PROMPT
    ...
    assert "watchlist" in SEED_PROMPT
    assert "不要把详细持仓继续写进 memory.md" in SEED_PROMPT
    assert "不要把具体自选 symbol 清单继续写进 memory.md" in SEED_PROMPT
