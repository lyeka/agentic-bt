...
src/agent/ - 持久投资助手（Kernel + runtime + session_store + tools + adapters: cli/telegram/im）
...
211 tests 全绿。agent 持久投资助手进入 Phase 2（runtime 统一组装 + SessionStore + IMDriver + Telegram polling 入口）。agenticbt 回测框架完成（确定性引擎 + 多资产 + bracket/limit/stop + 风控 + 11 工具）。项目重心继续在 Phase 2（Discord/Webhook/流式输出/定时任务）。
路线图：docs/roadmap.md
