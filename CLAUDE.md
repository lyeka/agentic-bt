...
Python 3.10+ · openai · pandas · pandas-ta · tushare · yfinance · python-dotenv · textual · pytest-bdd
...
src/
  ...
  agent/       - 持久投资助手 (Phase 2: runtime + session_store + IM driver + Telegram/Discord + TUI 终端界面)
.agents/
  skills/      - Agent 技能定义文件
...
tests/
  features/    - 19 个 Gherkin feature 文件（可执行规格说明）
...
src/agent/ - 持久投资助手（Kernel + context_ops + subagents + runtime + session_store + tools + adapters: cli/telegram/discord/im）
...
240+ tests 全绿。agent 持久投资助手 Phase 2（runtime 统一组装 + SessionStore + IMDriver + Telegram/Discord + LLMProvider 抽象 + SubAgent 子代理系统 + Skills 技能引擎）。Kernel 修复：_do_llm_call 统一入口 + _stream_complete 流式渲染（provider.complete 非流式 / client streaming 双轨）+ tc.id dataclass 属性访问 + 死代码清除。runtime.py 恢复 lazy import（market adapters 不再硬依赖 tushare/yfinance）。core/subagent.py 消除 agent.providers 循环依赖（provider 由调用方注入）。cli.py compact_history 参数对齐 provider=。TUI 终端界面（StreamingMarkdown 防抖 + SidebarPanel + 主题切换 + 异常展示 + 工具状态原地更新）。agenticbt 回测框架完成。美股数据接入完成（yfinance + Finnhub + Composite）。
路线图：docs/roadmap.md
